...
if psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -tAc \
    "SELECT 1 FROM information_schema.schemata WHERE schema_name='finances'" | grep -q 1; then
  echo "Schema 'finances' already exists; skipping init SQL."
  exit 0
fi

echo "Applying init_finances.sql as user '$POSTGRES_USER' to database '$POSTGRES_DB'..."
