...
from src.models.orm_models import Transaction as TransactionModel, Attachment, User
...
def get_user_by_username(db: Session, postgres_user: str) -> None:
    user = db.query(User).filter(User.username == postgres_user).first()
    if user:
        return user
